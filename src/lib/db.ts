...
// Banco de dados em memória / Fallback com todas as 10 músicas
const LOCAL_SONGS_DB: Record<string, Song> = {
  'blinding-lights': {
    id: 'blinding-lights',
    cardCode: 'CARD-001',
...
export async function getSongById(id: string): Promise<Song | null> {
  ...
  if (LOCAL_SONGS_DB[key]) return LOCAL_SONGS_DB[key];
  
  const foundByCode = Object.values(LOCAL_SONGS_DB).find(
    s => s.cardCode.toLowerCase() === key || 
         s.id.toLowerCase() === key || 
         s.title.toLowerCase().includes(key)
  );
  
  return foundByCode || LOCAL_SONGS_DB['blinding-lights'];
}
...
export async function getAllCards(): Promise<Song[]> {
  ...
  if (sql) {
    try {
      ...
      console.warn('Neon Postgres indisponível, retornando lista local:', err);
    }
  }
  return Object.values(LOCAL_SONGS_DB);
}
